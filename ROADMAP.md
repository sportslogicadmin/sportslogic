...
- Surface grade time-sensitivity in user-facing copy — "Graded at {timestamp} — odds may have moved since." A slip graded at 9am will not necessarily produce the same letter at 3pm because lines move; that's correct behavior, not a bug, but it's currently invisible to the user. Noted 2026-06-16 after a same-slip same-day grade discrepancy (Christian Walker HR prop, B/-0.6% EV at ~9:39am vs A-/+0.8% EV that afternoon) prompted a "is this a regression" question. Related: per-leg best_odds/fair_odds/true_prob aren't persisted to the Leg model at all (only team/market/line/odds/ev/grade/sport/isWeak are) — historical grades can't be fully re-attributed to "my book moved" vs "the sharp comparison market moved" after the fact. If Step C's Prisma migration (trueProb/impliedProb fields) lands, this becomes answerable; worth keeping in mind when that migration is scoped.
